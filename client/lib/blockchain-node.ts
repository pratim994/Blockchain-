...
export class BlockchainNode {
    ...
    async addBlock(newBlock : Block) : Promise<void> {
        ...
        const errorMessagePrefix = `Block "${newBlock.hash.substr(0,8)}" is rejected`;

        const previousBlockIndex = this._chain.findIndex(b => b.hash === newBlock.previousHash);

        if(previousBlockIndex < 0){
            throw new Error(`${errorMessagePrefix} - there is no block in the chain with the specified previous "${newBlock.previousHash.substr(0,8)}".`);
    ...
    export function randomDelay(maxMilliseconds: number = 100) : Promise<void> {
        ...
        return new Promise((resolve) => {
            setTimeout(() => resolve(), Math.floor(Math.random.random()*Math.floor(maxMilliseconds)));
