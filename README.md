...
```
Current:  1 process / node, in-memory
↓
Phase 1:  Worker threads for mining + LevelDB persistence
↓
Phase 2:  Horizontal scale — multiple client pods behind nginx
          WebSocket sticky sessions (IP hash or cookie)
↓
Phase 3:  P2P gossip protocol (libp2p) replaces hub-and-spoke WS
          Full node discovery, DHT-based peer routing
↓
Phase 4:  Sharding or Layer-2 (state channels) for throughput
```

### Throughput estimate

At difficulty 3 (3 leading zeros), a modern laptop mines a block in ~100ms. A 3-node network produces:

- ~10 blocks/second (race condition rate)
- After retargeting stabilizes to 5s/block: **0.2 blocks/second**
- At 10 transactions/block: **2 TPS**

Bitcoin: ~7 TPS. Ethereum (pre-Merge): ~15 TPS. Visa: ~24,000 TPS. This gap is why Layer-2 solutions (Lightning, Rollups) exist.

---

## Further Enhancements

1. **UTXO model**: Replace account balances with Bitcoin-style Unspent Transaction Outputs. Eliminates double-spend without a global state read.
2. **Mempool fee prioritization**: Miners select highest-fee transactions first (priority queue).
3. **Bloom filters**: SPV clients use Bloom filters to request only relevant transactions.
4. **Merkle proofs**: Expose an API endpoint to prove transaction inclusion without the full block.
5. **Metrics (Prometheus)**: Export `blocks_mined_total`, `mining_duration_seconds`, `connected_peers` etc.
6. **Signature aggregation (BLS)**: Aggregate multiple ECDSA signatures into one for multi-sig without O(n) verification cost.
7. **WebRTC P2P**: Replace WebSocket star topology with browser-to-browser P2P using WebRTC data channels.
...
| Topic | What to say |
|---|---|
| **Why clean architecture?** | "The core is a pure function library — I can unit test every consensus rule in 10ms with no mocks. I/O is a plugin." |
| **How do you handle forks?** | "Nakamoto consensus: when two chains diverge, all nodes converge to the longest valid chain. Mining aborts via AbortController when a peer wins the race." |
| **How does ECDSA work here?** | "Transactions are signed over {id, sender, recipient, amount, timestamp}. Verification uses the sender's public key embedded in the transaction — no certificate authority needed." |
| **Why Merkle trees?** | "They enable O(log n) transaction proofs. In production this powers SPV clients — light nodes that verify payments without downloading the full chain." |
| **How would you scale this?** | "Worker threads for CPU-bound mining, LevelDB for persistence, then replace hub-and-spoke WS with libp2p gossip for true decentralization." |
| **What's the CAP theorem trade-off?** | "Blockchain is CA under normal operation, P during network partitions. During a split, both partitions continue mining, creating a fork. Consistency is restored after reconnection via chain replacement." |
